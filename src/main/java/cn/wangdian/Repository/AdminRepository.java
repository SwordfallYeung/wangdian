...
public interface AdminRepository extends JpaRepository<Admin,String>,JpaSpecificationExecutor<Admin>{
    ...
    @Query("select o from Admin o where lower(o.id)=lower(:id) ")
    ...
    @Modifying
    @Query("update Admin o set o.status=:status where lower(o.id)=lower(:id) ")
    public void suoJieDing(@Param("id")Integer id,@Param("status")Integer status);

    @Modifying
    @Query("update Admin o set o.isDel=1 where lower(o.id)=lower(:id) ")
