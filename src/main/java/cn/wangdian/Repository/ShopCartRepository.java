...
//    public User selectByUsername(@Param("username") String username);
    ...
    @Query("select o from ShopCart o where lower(o.id)=lower(:id) ")
    public ShopCart findById(@Param("id") Integer id);

    @Query("select o.allSecondCost from ShopCart o where lower(o.id)=lower(:id) ")
    public float findAllSecondCostById(@Param("id") Integer id);

    @Query("select o.allProfits from ShopCart o where lower(o.id)=lower(:id) ")
    public float findAllProfitsById(@Param("id") Integer id);

    @Modifying
    @Query("update ShopCart o set o.isDel=1 where lower(o.id)=lower(:id) ")
...
//    public void updateOrderIdById(@Param("orderId") Integer orderId,@Param("shopCartId") Integer shopCartId);
    ...
    @Modifying
    @Query("update ShopCart o set o.status=:status where lower(o.id)=lower(:shopCartId) ")
