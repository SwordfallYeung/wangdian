...
<div data-role="page">
    ...
    <div data-role="main" class="ui-content">
        ...
        <div class="order-detail">
            <div class="order-status">订单状态：
                ...
                <c:if test="${orders.status==6}">已取消订单</c:if></div>
            <c:if test="${orders.kuaiDiDanHao!=null}"><div class="delivery-num">快递单号：<span>${orders.kuaiDiDanHao}</span></div></c:if>
            <div class="order-address">
                <div class="name-phone">
                    <div>收货人：<span>${orders.receiver}</span></div>
                    <div>${orders.receiverPhone}</div>
                </div>
                <div class="address-info">
                    收货地址：<span>${orders.address}</span>
                </div>
                <div>订单号：<span>${orders.shopOrderId}</span></div>
                <div>下单时间：<span>${orders.shopOrderTimeToString()}</span></div>
                <div>买家留言：<span>${orders.liuYan}</span></div>
            </div>
            ...
            <c:forEach items="${orders.shopCartList}" var="list">
                <div class="order-info">
                    ...
                    <div>
                       ...
                       <div class="parameter">商品规格：${list.detail}</div>
                        <span>商品单价：￥${list.secondCost}</span>
                        <span>数量：${list.count}</span>
                        <span>商品总价：￥${list.allSecondCost}</span>
                    </div>
                </div>
            </c:forEach>
            ...
            <c:if test="${orders.status==0}">
                <img src="${pageContext.request.contextPath}/frontAssets/images/alipay.png" style="width: 100%;border-bottom: 1px solid #ddd">
            </c:if>
        </div>
    </div>
    ...
    <div class="order-options">
        ...
        <c:if test="${orders.status==0}">
            ...
            <%--<a data-ajax="false" href="#"><button data-role="none" id="payBtn">付款</button></a>--%>
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersCancel?shopOrderId=${orders.shopOrderId}&userId=${userId}"><button data-role="none" id="delOrderBtn">取消订单</button></a>
        </c:if>
        <%--已取消订单，可以删除订单--%>
        <c:if test="${orders.status==6}">
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersDelete?id=${orders.id}&userId=${userId}"><button data-role="none" id="delOrderBtn">删除</button></a>
        </c:if>
        <%--已付款，卖家尚未发货，可以申请退款--%>
        <c:if test="${orders.status==1}">
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersToRefund?shopOrderId=${orders.shopOrderId}&userId=${userId}"><button data-role="none" id="refundsBtn">申请退款</button></a>
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersContact"><button data-role="none" id="contactBtn">联系卖家</button></a>
        </c:if>
        <%--已发货，买家可以确认收货--%>
        <c:if test="${orders.status==2}">
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersToConfirmReceipt?shopOrderId=${orders.shopOrderId}&userId=${userId}"><button data-role="none" id="confirmBtn">确认收货</button></a>
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersContact"><button data-role="none" id="contactBtn">联系卖家</button></a>
        </c:if>
        <%--已确认收货，交易完成，可以删除订单--%>
        <c:if test="${orders.status==3}">
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersDelete?id=${orders.id}&userId=${userId}"><button data-role="none" id="delOrderBtn">删除订单</button></a>
        </c:if>
        <%--申请退款中，可以撤销申请--%>
        <c:if test="${orders.status==4}">
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersToWithdraw?shopOrderId=${orders.shopOrderId}&userId=${userId}"><button data-role="none" id="recallBtn">撤销申请</button></a>
            <a data-ajax="false" href="${pageContext.request.contextPath}/ordersContact"><button data-role="none" id="contactBtn">联系卖家</button></a>
        </c:if>
        <%--已退款，可以删除订单--%>
...
<script>sessionStorage.setItem('freshFlag',"1");</script>
