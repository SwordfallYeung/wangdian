...
<div data-role="page">
    ...
    <div data-role="header" class="fixed-header">
        <a href="#" class="ui-btn" data-rel="back" id="returnBtn">返回</a>
        <h1>新增收货地址</h1>
        <a data-ajax="false" href="${pageContext.request.contextPath}/addressManagement?userId=${userId}" class="ui-btn">我的地址</a>
    </div>
    ...
    <div data-role="main" class="ui-content">
        <form method="post" action="${pageContext.request.contextPath}/addAddress" id="addAddressForm">
            <div class="ui-field-contain">
                <%--<input type="hidden" name="id" value="">--%>
                <input type="hidden" name="isDel" value="0">
                <input type="hidden" name="id" value="${userAddress.id}">
                <input type="hidden" name="status" <c:if test="${userAddress.status!=null}">value="${userAddress.status}"</c:if> <c:if test="${userAddress.status==null}">value="1"</c:if> >
