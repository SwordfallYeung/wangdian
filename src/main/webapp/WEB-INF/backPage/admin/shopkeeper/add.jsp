...
<div class="bjui-pageContent">
     <form action="${pageContext.request.contextPath}/admin/shopKeeper/add" data-toggle="validate" <c:if test="${type.equals('shopKeeper')}">data-callback="mycallback"</c:if> <c:if test="${!type.equals('shopKeeper')}">data-reload-navtab="true"</c:if> method="post">
                 ...
                 <tbody>
                       <tr>
                           <td colspan="2" style="width: 50%;">
                               ...
                               <input type="text" data-rule="required" id="username" <c:if test="${type.equals('shopKeeper')}">disabled</c:if> name="username" value="${shopKeeper.username}" size="20">
                               <c:if test="${!type.equals('shopKeeper')}"><a href="#" id="checkUsername" >检测用户名是否已存在</a></c:if>
                           </td>
                           <td colspan="2" >
                               <label class="control-label x100">昵称：</label>
                               <input type="text" data-rule="required" name="nickname" value="${shopKeeper.nickname}" size="20">
                           </td>
                       </tr>
                       <tr >
                           <td colspan="2" style="width: 50%;">
                           ...
                           <td colspan="2">
                               ...
                               <input type="text" data-rule="email" name="email" value="${shopKeeper.email}" size="20">
                           </td>
                       </tr>
                       ...
                       <tr>
                           ...
                           <tr>
                               <td colspan="2" style="width: 50%;" >
                                   <label class="control-label x100">店主网址：</label>
                                   <input type="text" disabled value="${shopKeeper.webUrl}" size="10"><span style="margin-top: 2px;">.${yuMing.yuming}</span>　(与用户名保持一致!!!)
                               </td>
                               <td colspan="2">
                           ...
                           <tr >
                               <td colspan="4" >
                                   ...
                                   <input type="radio" <c:if test="${type.equals('shopKeeper')}">disabled</c:if> name="status" data-toggle="icheck" data-label="正常" value="0" <c:if test="${shopKeeper.status==0}">checked</c:if><c:if test="${shopKeeper.status==null}">checked</c:if>/>
                                   <input type="radio" <c:if test="${type.equals('shopKeeper')}">disabled</c:if> name="status" data-toggle="icheck" data-label="锁定" value="1" <c:if test="${shopKeeper.status==1}">checked</c:if>/>
                               </td>
                           </tr>
                 </tbody>
             </table>
